...
void platformRadioInit(void)
{
    ...
    radio_dev = device_get_binding(CONFIG_OT_PLAT_IEEE802154_RADIO_DEVICE_NAME);
    __ASSERT_NO_MSG(radio_dev != NULL);

    radio_api = (struct ieee802154_radio_api *)radio_dev->driver_api;

    __ASSERT(radio_api->get_capabilities(radio_dev) & IEEE802154_HW_TX_RX_ACK,
        "Only radios with automatic ack handling are currently supported");
}
...
void platformRadioProcess(otInstance *aInstance) {
	if (sState == OT_RADIO_STATE_TRANSMIT)
	{
		...
		// the FCS size as radio driver adds CRC and increases frame
		// length on its own.
		...
		{
			otPlatDiagRadioTransmitDone(aInstance, &sTransmitFrame, result);
		}
		else
#endif
		{
            if (sTransmitFrame.mPsdu[0] & 0x20)
            {
                ...
                uint8_t ackPsdu[] = {0x02, 0x00, 0x00, 0x00, 0x00};
