...
.. code-block:: cmake
  ...
  include(CachingExternalProject)
...
function(_ep_write_gitclone_script script_filename source_dir git_EXECUTABLE git_repository git_tag git_sha git_ref git_remote_name git_cache_dir git_submodules git_shallow git_progress git_config src_name work_dir gitclone_infofile gitclone_stampfile tls_verify)
...
set(git_sha       \"${git_sha}\"      )
set(git_cache_dir \"${git_cache_dir}\")
set(git_ref       \"${git_ref}\"      )

if(git_sha STREQUAL \"\")
  set(git_sha_enabled 0)
else()
  set(git_sha_enabled 1)
endif()

if(git_cache_dir STREQUAL \"\")
  set(git_cache_dir_enabled 0)
else()
  set(git_cache_dir_enabled 1)
endif()

if(git_ref STREQUAL \"\")
  set(git_ref_enabled 0)
else()
  set(git_ref_enabled 1)
endif()
...
# The cache can only be used if both a cache directory and a git sha
# has been specified
if(git_cache_dir_enabled AND git_sha_enabled)
  set(cache 1)
else()
  set(cache 0)
endif()
...
# TODO: Test git_options.

# Make sure there is a git repo in the cache directory
if(cache)
  # Assume that the presence of a .git directory means that there is
  # already a git repository present
  if(NOT (EXISTS \${git_cache_dir}/.git))
    execute_process(
      COMMAND \"${git_EXECUTABLE}\" \${git_options} init \${git_cache_dir}
      RESULT_VARIABLE error_code
      )
    if(error_code)
      message(FATAL_ERROR An_unexpected_error_occurred)
    endif()
  endif()
endif()

# Check if the desired sha already exists in the cache
set(sha_exists_in_local_cache 0)
if(cache)
  execute_process(
    COMMAND \"${git_EXECUTABLE}\" \${git_options} cat-file -e \${git_sha}^{commit}
    WORKING_DIRECTORY \"\${git_cache_dir}\"
    RESULT_VARIABLE error_code
    )
  if(error_code EQUAL 0)
    set(sha_exists_in_local_cache 1)
  endif()
endif()

if(cache AND (NOT sha_exists_in_local_cache))
  # The SHA was not in the cache so we need to fetch it
  set(fetch_options)
  if(git_ref_enabled)
    # Limit what we download
    set(fetch_options \${git_ref} --depth=1)
  endif()

  # TODO: Retry loop since this is a network operation
  execute_process(
    COMMAND  \"${git_EXECUTABLE}\" \${git_options} fetch ${git_repository} \${fetch_options} # TODO: --origin \"${git_remote_name}\"
    WORKING_DIRECTORY \"${git_cache_dir}\"
    RESULT_VARIABLE error_code
    )

  # Possibilities now;
  # the SHA is present, but it's name is unknown and located in FETCH_HEAD,
  # the SHA is present, and it's name is known, but it's not located in FETCH_HEAD
  if(NOT git_sha) # TODO: Test
    execute_process(
      COMMAND  \"${git_EXECUTABLE}\" \${git_options} rev-parse FETCH_HEAD
      WORKING_DIRECTORY \"${git_cache_dir}\"
      OUTPUT_VARIABLE stdout
      RESULT_VARIABLE error_code
      )
    set(discovered_git_sha \${stdout})
  endif()

endif()

if(discovered_git_sha)
  set(sha \${discovered_git_sha})
elseif(git_sha)
  set(sha \${git_sha})
else()
  message(FATAL_ERROR Unexpected_error_occurred)
endif()

# TODO: Test if already tagged
...
if(NOT cache)
  ...
  while(error_code AND number_of_tries LESS 3)
    execute_process(
      COMMAND \"${git_EXECUTABLE}\" \${git_options} clone \${git_clone_options} --origin \"${git_remote_name}\" \"${git_cache_dir}\" \"${src_name}\"
      WORKING_DIRECTORY \"${work_dir}\"
      RESULT_VARIABLE error_code
      )
...
# TODO: Update? How will that work?
function(_ep_write_gitupdate_script script_filename git_EXECUTABLE git_tag git_sha git_ref git_remote_name git_cache_dir git_submodules git_repository work_dir)
...
if(git_tag STREQUAL \"\")
  set(git_tag_enabled 0)
else()
  set(git_tag_enabled 1)
endif()
...
if(NOT (git_tag_enabled OR git_ref_enabled OR git_sha_enabled))
  message(FATAL_ERROR \"Either tag, ref, or sha for git checkout must be specified.\")
endif()
...
if(git_sha_enabled)
  set(active_ref \${git_sha})
elseif(git_ref_enabled)
  set(active_ref \${git_ref})
else()
  set(active_ref \${git_tag})
endif()

# TODO: Replace all git_tag

unset(show_ref_output)
if(NOT git_sha_enabled)
  # Find out what kind of ref we have if we haven't explicitly
  # specified a SHA ref
  execute_process(
    COMMAND \"${git_EXECUTABLE}\" show-ref \${active_ref}
    WORKING_DIRECTORY \"${work_dir}\"
...
# If 'git_tag' is in the form <remote>/<tag> (i.e. origin/master) we
# must strip the remote from the tag.
...
# Find the SHA that we should update to. This will fail if the
# active_ref does not exist (it probably has not been fetched
# yet).
execute_process(
  COMMAND \"${git_EXECUTABLE}\" rev-list --max-count=1 \${active_ref}
  WORKING_DIRECTORY \"${work_dir}\"
  RESULT_VARIABLE error_code
...
function(_ep_write_downloadfile_script script_filename REMOTE LOCAL timeout no_progress hash tls_verify tls_cainfo userpwd http_headers netrc netrc_file)
  ...
  # * HTTP_HEADERS_ARGS
  configure_file(
      "${_ExternalProject_SELF_DIR}/CachingExternalProject-download.cmake.in"
      "${script_filename}"
      @ONLY
  )
endfunction()
...
function(_ep_write_verifyfile_script script_filename LOCAL hash)
  ...
  # * LOCAL
  configure_file(
      "${_ExternalProject_SELF_DIR}/CachingExternalProject-verify.cmake.in"
      "${script_filename}"
      @ONLY
  )
endfunction()
...
function(_ep_add_download_command name)
  ...
  elseif(git_repository)
    ...
    get_property(git_cache_dir  TARGET ${name} PROPERTY _EP_GIT_CACHE_DIR)
    get_property(git_sha        TARGET ${name} PROPERTY _EP_GIT_SHA)
    get_property(git_ref        TARGET ${name} PROPERTY _EP_GIT_REF)
    ...
    _ep_write_gitclone_script(${tmp_dir}/${name}-gitclone.cmake ${source_dir}
      ${GIT_EXECUTABLE} ${git_repository} ${git_tag} "${git_sha}" "${git_ref}" ${git_remote_name} "${git_cache_dir}" "${git_submodules}" "${git_shallow}" "${git_progress}" "${git_config}" ${src_name} ${work_dir}
...
function(_ep_add_update_command name)
  ...
  elseif(git_repository)
    ...
    get_property(git_sha TARGET ${name} PROPERTY _EP_GIT_SHA)
    get_property(git_ref TARGET ${name} PROPERTY _EP_GIT_REF)
    if(NOT (git_tag OR git_sha OR git_ref))
      set(git_tag "master")
    endif()
    get_property(git_cache_dir   TARGET ${name} PROPERTY _EP_GIT_CACHE_DIR)
    get_property(git_remote_name TARGET ${name} PROPERTY _EP_GIT_REMOTE_NAME)
    ...
    _ep_write_gitupdate_script(${tmp_dir}/${name}-gitupdate.cmake
      ${GIT_EXECUTABLE} "${git_tag}" "${git_sha}" "${git_ref}" ${git_remote_name} "${git_cache_dir}" "${git_submodules}" ${git_repository} ${work_dir}
      )
